...
/// In practice, the maximum number of validators might be even lower depending
/// on the block gas limit of the network (which can be higher in some EVM L2s).
...
interface Quorum is EpochManager {
    ...
    /// @notice Vote for a post-epoch state.
    /// @param epochIndex The epoch index
    /// @param postEpochStateRoot The post-epoch state root
