...
// Reassemble the original string from its constituent parts
const reassembledVersion = `${major}.${minor}.${patch}`
...
if (nodeJsPackageVersion !== reassembledVersion) {
    console.error(`Error: Failed to correctly parse version ${nodeJsPackageVersion}.`);
    console.error(`Reassembled version string ${reassembledVersion} differs from original.`);
    process.exit(1);
}
...
// Check if the RegExp matches any occurrence in contract content
...
if (contractContent === newContractContent) {
    console.log("Contracts version is already in-sync.");
} else {
