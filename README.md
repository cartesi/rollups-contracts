...
If you are interested in taking a look at the off-chain part, please, head over to the [`cartesi/rollups-node`](https://github.com/cartesi/rollups-node) repository.
...
This repository uses [Yarn](https://classic.yarnpkg.com/en/docs/install) to manage npm packages.
...
_(Optional)_ If you plan to run the on-chain tests, there still some setup left to do.
Assuming you are on the `onchain` directory, you may run the following commands.
...
Now, you may want to test the on-chain code with [Forge](https://book.getfoundry.sh/getting-started/installation).
...
_(Optional)_ If you modify the smart contracts, you may need to update the proofs used in the tests.
Assuming you are on the `onchain/rollups` directory, please, run the following command.
...
To build the [Docker](https://docs.docker.com/get-docker/) images, run the following command.
