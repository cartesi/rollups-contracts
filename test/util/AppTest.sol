...
abstract contract AppTest is Test {
        ...
        external
    {
        for (uint256 epochIndex; epochIndex < postEpochOutputsRoots.length; ++epochIndex)
        {
            ...
            address epochFinalizer = _closeEpoch(_app, epochIndex);
    ...
    function _closeEpoch(App app, uint256 epochIndex)
        internal
    ...
    function _makeOutputsValid(App app) internal {
        // We store the epoch finalizer so that we can finalize the epoch later
        uint256 epochIndex = app.getFinalizedEpochCount();
        address epochFinalizer = _closeEpoch(app, epochIndex);
        ...
        _preFinalizeEpoch(epochIndex, epochFinalizer, postEpochStateRoot);
        app.finalizeEpoch(epochIndex, outputsRoot, proof);
        assertTrue(app.isOutputsRootFinal(outputsRoot));
    }
